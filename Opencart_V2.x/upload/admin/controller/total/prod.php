<?php
class ControllerTotalProd extends Controller {
	private $error = array();

	public function index() {

		// load all language variables
		$data = $this->load->language('total/prod');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('total/prod', 'token=' . $this->session->data['token'], true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('total/prod', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('total/prod', 'token=' . $this->session->data['token'], true);
		...
		$this->response->setOutput($this->load->view('total/prod.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'total/prod')) {
