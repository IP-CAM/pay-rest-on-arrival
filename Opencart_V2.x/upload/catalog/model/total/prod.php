<?php
class ModelTotalProd extends Model {
	public function getTotal(&$total_data, &$total, &$taxes) {

		if($total < $this->config->get('prod_total')) {
			return;
		}
		...
		if (isset($this->session->data['prod_opt']) 
			...
			$prod_methods = $this->config->get('prod_methods');
			$prod_title = $this->config->get('prod_text_total');
			foreach($prod_methods as $method) {
				if($user_method_code == $method['payment_method']) {

					$this->load->language('total/prod');
					...
					if($payment_type == 'percentage' && $payment_amount > 0 && $payment_amount < 100){
						$prod_amount = round(($payment_amount * $total) / 100, 2, PHP_ROUND_HALF_UP);

						$title = sprintf($language_text, $payment_amount . ($payment_type == 'percentage'? '%' : ''));

					} else if($payment_type == 'flat') {
						$prod_amount = min($total, $payment_amount);
					...
					if($prod_amount > 0){
						$total_data[] = array(
							...
							'title'      => $title,
							'value'      => - $prod_amount,
							'sort_order' => $this->config->get('prod_sort_order') + 0.01
						);

						$total = $total - $prod_amount;
					}
					break;
				}
			}
